...
const yourDate = new Date("2021-10-06T00:00:00"); // <-- bạn đổi ngày tại đây
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (dateElement) {
    dateElement.textContent = Math.floor(
      (new Date() - yourDate) / 1000 / 60 / 60 / 24
    );
  }
  ...
  function updateClock() {
    ...
    if (hoursElement) hoursElement.textContent = hrs < 10 ? "0" + hrs : hrs;
    if (minutesElement) minutesElement.textContent = mins < 10 ? "0" + mins : mins;
    if (secondsElement) secondsElement.textContent = secs < 10 ? "0" + secs : secs;
  }
  ...
  if (developControl && polaroidPhoto) {
    let currentPhotoIndex = 1;
    const totalPhotos = 7; // có 7 ảnh: anh_1.png → anh_7.png

    developControl.addEventListener("click", () => {
      polaroidPhoto.classList.add("is-developed");

      // đổi ảnh
      polaroidPhoto.style.backgroundImage = `url('./anh_${currentPhotoIndex}.png')`;

      // cập nhật ảnh tiếp theo
      currentPhotoIndex++;
      if (currentPhotoIndex > totalPhotos) currentPhotoIndex = 1;
    });
  }

  if (resetControl && polaroidPhoto) {
